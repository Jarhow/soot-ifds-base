package basic.dataflow;

import basic.cfg.Node;
import basic.cfg.Path;
import soot.Unit;
import soot.Value;
...
public class DataFlow {
     ...
     */
    public static HashSet<Node> findAllDefUnitAffectThisValue(Node sourceNode, ValueBox valueBox) {//反向搜索数据的传播
        HashSet<Event> processedEvent = new HashSet<>();
        EventQueue waitForProcessEvent = new EventQueue();
        waitForProcessEvent.add(new Event(sourceNode, valueBox));
        HashSet<Node> nodeAffect = new HashSet<>();
        while (!waitForProcessEvent.isEmpty()) {
            Event event = waitForProcessEvent.poll();
            Node node = event.node;
            ValueBox valuebox = event.valueBox;
...
//                        System.out.println(preNode.unit+": "+valuebox.getValue());
                        //1、是被new出来的
...
//
                        ...
                        } else if (((AssignStmt) preNode.unit).containsInvokeExpr()) {
                            //6、属于返回值，我们要查看返回值
